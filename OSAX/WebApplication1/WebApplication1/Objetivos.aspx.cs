...
namespace WebApplication1
{
    public partial class Objetivo : System.Web.UI.Page
    {
        ...
        protected void registrar(Object sender, EventArgs e)
        {
            ...
            vals.Add("F_ID", metas.SelectedValue);
            vals.Add("nombre", nombre);
            vals.Add("ID_Caso", Session["idcaso"].ToString());
            if (dbcontroller.addRecord("Objetivos", vals))
        ...
        private void fillDropDown(DropDownList ddl)
        {
            Connection cn = new Connection();
            DBM bdm = new DBM(cn);
            string query = "select ID,nombre from Metas" + " where ID_Caso='" + Session["idcaso"].ToString() + "'";
