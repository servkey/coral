...
namespace WebApplication1
{
    public partial class Objetos : System.Web.UI.Page
    {
        ...
        protected void registrar(Object sender, EventArgs e)
        {
            ...
            vals.Add("F_ID", categorias.SelectedValue);                        
            vals.Add("nombre", nombre);
            vals.Add("ID_Caso", Session["idcaso"].ToString());
            if (dbcontroller.addRecord("Objetos", vals))
        ...
        private void fillDropDown(DropDownList ddl)
        {
            Connection cn = new Connection();
            DBM bdm = new DBM(cn);
            string query = "select ID,nombre from Categorias" + " where ID_Caso='" + Session["idcaso"].ToString() + "'";
